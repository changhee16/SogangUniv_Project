...
def rnn_single_step(current_input:torch.Tensor, prev_hidden:torch.Tensor, hh_weight:torch.Tensor, ih_weight:torch.Tensor, bias:torch.Tensor) -> torch.Tensor:
  ...
  '''
  current_hidden = torch.tanh(ih_weight @ current_input + hh_weight @ prev_hidden + bias)
...
def rnn_for_entire_timestep(input_seq:torch.Tensor, prev_hidden:torch.Tensor, hh_weight:torch.Tensor, ih_weight:torch.Tensor, bias:torch.Tensor) -> tuple:
  ...
  for i in range(input_seq.size(0)):
     ...
     if i == 0:
      res = prev_hidden.unsqueeze(0)
     else:
      res = torch.cat([res, prev_hidden.unsqueeze(0)], dim = 0)
  return (res, res[input_seq.size(0) - 1])
...
class MelodyDataset:
  ...
  def __getitem__(self, idx):
    ...
    TODO: Complete this function
    '''
    melody = torch.tensor([self.pitch2idx['start'], self.dur2idx['start']]).unsqueeze(0)
    shift_melody = torch.tensor([self.pitch2idx[self.dataset[idx][0][1]], self.dur2idx[self.dataset[idx][0][2]]]).unsqueeze(0)
    for i in range(len(self.dataset[idx])):
        tmp_1 = torch.tensor([self.pitch2idx[self.dataset[idx][i][1]], self.dur2idx[self.dataset[idx][i][2]]]).unsqueeze(0)
        melody = torch.cat([melody, tmp_1], dim = 0)
        if i < len(self.dataset[idx]) - 1:
          tmp_2 = torch.tensor([self.pitch2idx[self.dataset[idx][i + 1][1]], self.dur2idx[self.dataset[idx][i + 1][2]]]).unsqueeze(0)
          shift_melody = torch.cat([shift_melody, tmp_2], dim = 0)
    shift_melody = torch.cat([shift_melody, torch.tensor([self.pitch2idx['end'], self.dur2idx['end']]).unsqueeze(0)], dim = 0)
    return melody, shift_melody
...
class MelodyLanguageModel(nn.Module):
  def __init__(self, hidden_size, embed_size, vocabs):
    ...
    self.rnn = nn.GRU(self.embed_size * 2, self.hidden_size, self.num_layers, batch_first=True)
  ...
  def get_concat_embedding(self, input_seq):
    ...
    if input_seq.ndim == 2:
      input_seq.usqeeze(0)
    concat_embedding = torch.cat([self.pitch_embedder(input_seq[:,:,0]), self.dur_embedder(input_seq[:,:,1])], dim = -1)
  ...
  def forward(self, input_seq:torch.LongTensor):
    ...
    if isinstance(input_seq, torch.Tensor): # If input is an ordinary tensor
      ...
      concatenated_embed = self.get_concat_embedding(input_seq)
      rnn_output, _ = self.rnn(concatenated_embed)
      logits = self.final_layer(rnn_output)
      pitch_logits, dur_logits = torch.split(logits, [self.num_pitch, self.num_dur], dim = -1) #마지막 차원인건지 어케 알지
      pitch_dist = torch.softmax(pitch_logits, dim = -1)
      dur_dist = torch.softmax(dur_logits, dim = -1)
      ...
      # Caution! You have to get separately softmax-ed pitch and duration
      # Because you have to pick one pitch and one duration from the probability distribution
      padded_seq, batch_lengths = pad_packed_sequence(input_seq, batch_first=True)
      #print(batch_lengths)
      concatenated_embed = self.get_concat_embedding(padded_seq)
      #print(padded_seq.shape)
      #print(concatenated_embed.shape)
      rnn_output, _ = self.rnn(concatenated_embed)
      logits = self.final_layer(rnn_output)
      #print(logits.shape)
      pitch_logits, dur_logits = torch.split(logits, [self.num_pitch, self.num_dur], dim = -1) #마지막 차원인건지 어케 알지
      #print(pitch_logits.shape)
      pitch_dist = torch.softmax(pitch_logits, dim = -1)
      dur_dist = torch.softmax(dur_logits, dim = -1)
      pitch_dist = pack_padded_sequence(pitch_dist, batch_lengths, enforce_sorted=False, batch_first=True) #다시 되돌리기 
      dur_dist = pack_padded_sequence(dur_dist, batch_lengths, enforce_sorted=False, batch_first=True)
      #pass # Delete this after you complete the code
    else:
...
def get_nll_loss(prob_distribution, correct_class):
  ...
  loss = -torch.log(prob_distribution.gather(1, correct_class.view(-1, 1)).squeeze())
  return loss
...
def get_loss_for_single_batch(model, batch, device):
  ...
  duration_loss = get_nll_loss(dur_dist.data, shifted_melody_batch.data[:,1])
  loss = (pitch_loss.mean() + duration_loss.mean()) / 2
  return loss
...
def get_initial_input_and_hidden_state(model, batch_size=1):
  ...
  start_pitch_index = model.pitch2idx['start']
  start_duration_index = model.dur2idx['start']
    
  initial_input_vec = torch.zeros(batch_size, 1, 2)
  initial_input_vec[:, 0, 0] = start_pitch_index
  initial_input_vec[:, 0, 1] = start_duration_index
...
def predict_single_step(model, cur_input, prev_hidden):
  ...
  TODO: Complete this function
  '''
  concat = model.get_concat_embedding(cur_input.to(torch.long))
  rnn_output, hidden = model.rnn(concat, prev_hidden)
  cur_output = model.final_layer(rnn_output)
  pitch_logits, dur_logits = torch.split(cur_output, [model.num_pitch, model.num_dur], dim = -1)
  
  pitch_dist = torch.softmax(pitch_logits, dim = -1)
  dur_dist = torch.softmax(dur_logits, dim = -1)
  pitch_dist = pitch_dist.squeeze(0)
  dur_dist = dur_dist.squeeze(0)
  pitch_category_idx = pitch_dist.multinomial(num_samples=1, replacement=False)
  duration_category_idx = dur_dist.multinomial(num_samples=1, replacement=False)
  cur_output = torch.zeros(1, 1, 2)
  cur_output[0][0][0] = pitch_category_idx
  cur_output[0][0][1] = duration_category_idx

  return cur_output, hidden
...
def is_end_token(model, cur_output):
  ...
  pitch_idx = cur_output[0, 0, 0]
  dur_idx = cur_output[0, 0, 1]
  pitch = model.idx2pitch[pitch_idx.int()]
  duration = model.idx2dur[dur_idx.int()]
  is_end_token = (pitch == 'end') or (duration == 'end')
  return is_end_token
...
def generate(model, random_seed=0):
  ...
  Write your code from here
  '''
  initial_input_vec, initial_hidden = get_initial_input_and_hidden_state(model, batch_size=1)
  generated_note_sequence = [] #list 

  cur_input = initial_input_vec
  prev_hidden = initial_hidden
  while not is_end_token(model, cur_input):
      cur_output, prev_hidden = predict_single_step(model, cur_input, prev_hidden)
      if cur_output[0][0][0] < 2 or cur_output[0][0][1] < 2:
        break  
      generated_note_sequence.append(cur_output[0][0].tolist())
      cur_input = cur_output
  generated_note_sequence = torch.LongTensor(generated_note_sequence)
...
def convert_idx_pred_to_origin(pred:torch.Tensor, idx2pitch:list, idx2dur:list):
  ...
  for i in range(0, pred.shape[0]):
    if pred[i][0] == 2 or pred[i][1] == 2:
      converted_out.append([0,0])
    else:
      converted_out.append([idx2pitch[pred[i][0]], idx2dur[pred[i][1]]])
  converted_out = torch.Tensor(converted_out)
...
def convert_pitch_dur_to_note_representation(pitch_dur:torch.LongTensor):
  ...
  note_repr = []
  start_timestep = 0
  for i in range(pitch_dur.shape[0]):
    note_repr.append([start_timestep, pitch_dur[i][0], pitch_dur[i][1], 64])
    start_timestep = start_timestep + pitch_dur[i][1]
  note_repr = np.array(note_repr, int)
  return note_repr
