...
def hz_to_midi_pitch(frequency):
  ...
  p = (log(frequency / 440) / log(2)) * 12 + 69
  return p
...
def generate_multi_pitch_tone(pitch_list, amp_list, duration, sr):
  ...
  for i in range(len(pitch_list)):
    example = make_sine_wave(midi_pitch_to_hz(pitch_list[i]), amp_list[i], duration, sr)
    output += example
  output /= len(pitch_list)
  return output
...
def make_major_scale_hz_sequence(midi_pitch):
  ...
  for i in range(8):
    if (i == 3 or i == 7):
      ...
      pitch += 2
    major.append(midi_pitch_to_hz(pitch))
  return major
...
def generate_sequence_of_pitch(alist_of_hz, duration, sr):
  ...
  for i in alist_of_hz:
    example = make_sine_wave(i, 1, duration, sr)
    output = torch.cat((output, example))
  return output
...
def make_just_intonation_triad_in_midi_pitch(fundamental_pitch):
  ...
  # TODO: 주어진 입출력 형식과 내용을 만족하는 함수를 완성하시오
  hz = midi_pitch_to_hz(fundamental_pitch)
  third_midi = hz_to_midi_pitch(hz * 5/4)
  five_midi = hz_to_midi_pitch(hz * 3/2)
  return [fundamental_pitch, third_midi, five_midi]
...
class ShepardToneGenerator:
  ...
  def define_pitch(self):
    ...
    for i in range(24):
     pitch = 60 + i
     output[i] = torch.tensor([pitch, pitch + 4, pitch + 7])
    return output
  ...
  def define_amplitude(self):
    ...
    for i in range(24):
      for j in range(3):
        if j == 2:
          freq_diff = i + 1
          log_amp = -abs(log(freq_diff))
        elif j == 0:
          freq_diff = 24 - i
          log_amp = -abs(log(freq_diff))
        elif j == 1:
          log_amp = 0
        output[i][j] = math.exp(log_amp / 3)
    return output
...
class SpectrogramConverter:
  ...
  def get_spectrogram(self, audio):
    ...
    out_spec = []
    for i in range(self.n_fft,len(audio) + 1,self.hop_size):
      start_i = i - self.n_fft
      sliced_samples = audio[start_i:i]
      windowed_samples = self.window_tensor * sliced_samples
      fft_result = torch.fft.fft(windowed_samples)
      out_spec.append(fft_result.abs()[:self.n_fft // 2 + 1]** self.power)
      #abs() 사용 이유 -> 크기값만 저장, 반 자르는 이유 -> 대칭되어 나와서. 
  ...
  def time_bin_to_second(self, bin_index):
    ...
    hop_time = hop_size / self.sr
    return hop_time * bin_index
  ...
  def convert_spec_to_mel_spec(self, spec):
    ...
    mel_spec = torch.mm(spec.transpose(0, 1), self.mel_fb)
  ...
  def __call__(self, audio):
    ...
    spec = self.get_spectrogram(your_audio)
    mel = self.convert_spec_to_mel_spec(spec)
    db = self.amplitude_to_db(mel)
    return db
